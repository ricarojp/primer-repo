    ...
    var colorPicker = function ($document) {
        return {
            ...
            link: function ($scope, element, attrs, control) {
                ...
                function watchColorPickerAlphaCase(newValue, oldValue) {
                ...
                $scope.$watch('colorPickerAlpha', watchColorPickerAlphaCase);
                $scope.$watch('colorPickerCase', watchColorPickerAlphaCase);

                function watchColorPickerSwatch(newValue, oldValue) {
                    if (newValue !== undefined) {
                        $scope.initConfig();
                    }
                };

                $scope.$watch('colorPickerSwatchPos', watchColorPickerSwatch);
                $scope.$watch('colorPickerSwatchBootstrap', watchColorPickerSwatch);
                $scope.$watch('colorPickerSwatchOnly', watchColorPickerSwatch);
                $scope.$watch('colorPickerSwatch', watchColorPickerSwatch);
                $scope.$watch('colorPickerPos', watchColorPickerSwatch);

                //---------------------------
                // Update Positions And Colors On Elements
                //---------------------------

                function updateSwatchColor(newValue) {
                    ...
                    el.css({
                        ...
                        'border': '1px solid '+tinycolor(newValue).darken(10).toString()
                    });
                }

                $scope.$watch('swatchColor', updateSwatchColor);
...
angular.module('color.picker').run(['$templateCache', function($templateCache) {
    $templateCache.put('template/color-picker/directive.html',
        ...
        '       <input class="color-picker-input form-control" type="text" ng-model="ngModel" ng-change="onChange($event)" size="7" ng-focus="show()" ng-class="{\'color-picker-input-swatch\': config.swatch && !config.swatchOnly && config.swatchPos === \'left\'}" placeholder="Default">\n' +
